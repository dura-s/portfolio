type SectionId = "home" | "about" | "projects" | "design" | "contact";

const navLinks = document.querySelectorAll<HTMLAnchorElement>(
  "header .nav a[data-section]"
);
...
navLinks.forEach((link) => {
  const targetId = link.dataset.section as SectionId | undefined;
  if (!targetId) return;
...
const observer = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      ...
      const correspondingLink = document.querySelector<HTMLAnchorElement>(
        `header .nav a[data-section="${id}"]`
      );
...
// Projects (rendered from TypeScript)
type ProjectLanguage = "TypeScript" | "JavaScript" | "HTML" | "CSS" | "Other";
...
const projects: Project[] = [
  {
    ...
    name: "NOVA",
    description:
      "Smart learning platform connecting teachers and students with role-based access, group chat, schedules, and more.",
    language: "TypeScript",
  ...
  },
  {
    ...
    name: "women-security",
    description:
      "TypeScript project focused on women’s security — built as a learning project with a real-world purpose.",
    language: "TypeScript",
  ...
  },
  {
    ...
    description:
      "Dorm maintenance UI for submitting requests and tracking progress. Clean layout and component consistency.",
    language: "CSS",
  ...
  },
  {
    ...
    name: "portfolio",
    description:
      "My personal portfolio website — modern design, smooth animations, and a strong focus on readability.",
    language: "HTML",
...
const projectEmpty = document.querySelector<HTMLElement>("#project-empty");
const projectSearch = document.querySelector<HTMLInputElement>("#project-search");
const projectFilters = document.querySelector<HTMLElement>("#project-filters");

const modal = document.querySelector<HTMLElement>("#project-modal");
const modalTitle = document.querySelector<HTMLElement>("#modal-title");
const modalDesc = document.querySelector<HTMLElement>("#modal-desc");
const modalTags = document.querySelector<HTMLElement>("#modal-tags");
const modalLinks = document.querySelector<HTMLElement>("#modal-links");

let activeLanguage: ProjectLanguage | "All" = "All";
let searchQuery = "";
...
function openModal(project: Project) {
  if (!modal || !modalTitle || !modalDesc || !modalTags || !modalLinks) return;
...
function createLink(label: string, href: string, isAccent = false) {
  const a = createEl("a", "link");
  if (isAccent) a.style.borderColor = "var(--accent-soft)";
  a.textContent = label;
  a.href = href;
  a.target = "_blank";
...
function renderFilters() {
  ...
  const languages: Array<ProjectLanguage | "All"> = [
    "All",
  ...
  languages.forEach((lang) => {
    ...
    if (lang === activeLanguage) btn.classList.add("active");
...
function getFilteredProjects(): Project[] {
  ...
  return projects.filter((p) => {
    ...
    const text =
      `${p.name} ${p.description} ${p.language} ${p.tags.join(" ")}`.toLowerCase();
    const qOk = !q ? true : text.includes(q);
    return langOk && qOk;
  });
}

function renderProjects(list: Project[]) {
  if (!projectGrid || !projectEmpty) return;
...
if (modal) {
  ...
  const closeButtons = modal.querySelectorAll<HTMLElement>("[data-close='true']");
